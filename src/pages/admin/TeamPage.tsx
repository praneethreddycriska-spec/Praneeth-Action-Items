...
import { Plus, Copy, Power, KeyRound, X } from 'lucide-react'
import { toast } from 'sonner'
import { useTeamMembers, useInviteTeamMember, useSetTeamMemberActive, useResetTeamMemberPassword, useMemberCommunities, useSetMemberCommunity } from '@/hooks/useTeam'
...
export default function TeamPage() {
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => { setOpen(false); setInviteLink(null) }}>
          <div className="glass-strong w-full max-w-sm rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => { setOpen(false); setInviteLink(null) }}><X size={16} /></button>
            </div>
            ...
            {inviteLink ? (
              <div className="space-y-3">
                ...
                <button onClick={() => { setOpen(false); setInviteLink(null) }} className="w-full rounded-xl bg-primary py-2 text-sm font-semibold text-primary-foreground">Done</button>
              </div>
            ) : (
