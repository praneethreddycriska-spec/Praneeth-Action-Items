-- admin_invite_team_member previously swallowed a failed generate_link call
-- (e.g. Supabase's email rate limit) and returned invite_link: null with no
-- indication why — the UI had no way to tell the difference between "link
-- generation genuinely failed" and just render the blank form again. Now
-- returns invite_link_error so the UI can show a real message.
...
begin
  ...
  v_link_body := jsonb_build_object('type', 'invite', 'email', trim(p_email), 'redirect_to', v_base_url || '/set-password');
