-- Configurable app base URL, used to build correct password-setup /
-- password-reset redirect links (was hardcoded to GoTrue's default
-- http://localhost:3000, which this app never runs on).
insert into public.system_settings (key, value)
values ('app_base_url', to_jsonb('http://localhost:5173'::text))
...
begin
  ...
  select * into v_link_response from extensions.http((
    'POST', v_project_url || '/auth/v1/admin/generate_link',
    array[extensions.http_header('apikey', v_service_key), extensions.http_header('Authorization', 'Bearer ' || v_service_key)],
    'application/json', jsonb_build_object('type', 'recovery', 'email', trim(p_email), 'redirect_to', v_base_url || '/set-password')::text
  )::extensions.http_request);
